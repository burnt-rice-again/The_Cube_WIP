...
local FilterStringToNum =
{
	...
	v_setnum        = 35, v_maxrange      = 36, v_mothership    = 37,
}
...
function FilterEntity(bounds_entity, e, filters)
	...
	for i=1, #filters, 2 do
		...
		elseif fnum < 30 then
			...
			elseif fnum == 37 then -- v_mothership
				ok = e.id == "f_mothership"
...
function FreeplaySpawnPlayerAlien(faction, loc)
	...
	----- BUILDINGS
	local alien = Map.CreateEntity(faction, "f_alien_feeder")
	alien:Place(loc.x+3, loc.y-3)

	alien = Map.CreateEntity(faction, "f_alien_extractor")
	alien:Place(loc.x-4, loc.y-4)

	----- WORKERS
	local alien = Map.CreateEntity(faction, "f_alien_worker")
	alien.disconnected = false
	alien:Place(loc.x + 6, loc.y)
	alien = Map.CreateEntity(faction, "f_alien_worker")
	alien.disconnected = false
	alien:Place(loc.x+3, loc.y)

	----- MINERS
	alien = Map.CreateEntity(faction, "f_alien_miner")
	alien.disconnected = false
	alien:Place(loc.x, loc.y-5)
	alien = Map.CreateEntity(faction, "f_alien_miner")
	alien.disconnected = false
	alien:Place(loc.x+5, loc.y-5)

	----- SCOUT
	alien = Map.CreateEntity(faction, "f_alien_scout")
	alien.disconnected = false
	alien:Place(loc.x+8, loc.y)
...
function GetBugCountsForLevel(player_level, total_bugs, was_forced)
	...
	for i=1,(total_bugs or 1) do
		...
		elseif r < 41 then -- "f_gastarias2"
			...
			if player_level > 58 and rnd > 0.7 then -- "f_tripodonte1"
				bug_counts[13] = bug_counts[13] + 1
			elseif player_level > 80 and rnd > 0.6 then -- "f_tetrapuss1"
				bug_counts[12] = bug_counts[12] + 1
			elseif player_level > 64 and rnd > 0.45 then -- "f_larva1"
				bug_counts[11] = bug_counts[11] + 1
			elseif player_level > 85 and rnd < 0.25 and r < 80 and not was_forced then -- "f_worm1"
