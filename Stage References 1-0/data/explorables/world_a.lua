...
Frame:RegisterFrame("f_world_big_daikon", {
	size = "Mission", race = nil, index = 999, name = "Super Silica Tree",
...
function c_big_daikon_trigger:on_take_damage(comp, amount, damager)
	...
	if comp_owner_health > halfhealth and (comp_owner_health - amount) <= halfhealth then
	...
	-- final hit
	if (comp_owner_health - amount) <= 0 then
