...
function GetFactionBehaviorAsm(faction_or_comp, revid)
	...
	for inst_idx,inst in ipairs(code) do
		...
		local arg_defs = inst_def.var_args and inst_def:var_args(inst, code, library, true) or inst_def.args
		if inst_def.make_asm then asminst[#asminst + 1] = inst_def.make_asm(inst, code) end
		for i=1,(arg_defs and #arg_defs or 0) do
			local val, is_exec, asmarg = inst[i], (arg_defs[i][1] == 'exec')
			local val_type = type(val)
			if is_exec then
				-- exec jump, calculate absolute target address
				asmarg = (val == nil and inst_idx < #code and inst_idx + 1 or val_type == "number" and val <= #code and val or false)
			...
			elseif val_type == "table" and val.fr then
				...
				for j,v in ipairs(fregs) do if freg == v then asmarg = -99 - j break end end
...
function SetBehavior(comp, id, debug, counter, breakpoints)
	...
	if not asm then
		if old_state then -- Keep the referenced behavior, erase runtime state
			old_state.revid, old_state.stk, old_state.counter, old_state.lastcounter, old_state.mem, old_state.blocks, old_state.returns, old_state.debug, old_state.limit, old_state.unlock_continue, old_state.laststep, old_state.lasttick = nil
...
local function UnpackOldCompactedItemToLibraryTable(tbl, ttype, out, sub_mapping, sub_id) -- tbl will get modified by this (caller needs to Tool.Copy if needed)
	...
	elseif ttype == 'C' then -- behavior
		...
		for inst_idx,inst in ipairs(tbl) do
			if inst.sub and inst.sub ~= 0 then -- legacy behaviors used 0 to reference self
...
local function LibraryApplyMapping(item, mapping)
	...
	if item_type == 'B' then
		for comp_idx,bpcomp,payload,payload_base_id in iblueprintcomponents(item) do
			if payload_base_id == 'c_behavior' or payload_base_id == 'c_fabricator' then
				...
				if mapped == nil then print('[LibraryApplyMapping] library import of blueprint has missed mapping from blueprint ' .. tostring(payload)) end
	...
	elseif item_type == 'C' then
		for _,inst in ipairs(item) do
			if inst.sub and inst.sub ~= 0 then -- legacy behaviors used 0 to reference self
				local mapped = mapping[inst.sub]
				if mapped == nil then print('[LibraryApplyMapping] library import of behavior has missed mapping from behavior ' .. tostring(inst.sub)) end
				inst.sub = mapped or nil -- convert false (explicitly ignore) to nil
			elseif inst.bp then
				local mapped = mapping[inst.bp]
				if mapped == nil then print('[LibraryApplyMapping] library import of behavior has missed mapping from blueprint ' .. tostring(inst.bp)) end
		...
		if entity.is_construction then
			...
			if not frame_def.type then -- not for walls/gates
				...
				local function AddReg(idx, map, skip_val)
					...
					if not reg_id then reg_entity = reg.raw_entity if not reg_entity then reg_coord = reg.coord end end
					...
					if (idx == -FRAMEREG_GOTO or idx == -FRAMEREG_STORE) and entity:RegisterQueueLength(idx) > 1 then
						...
						table.remove(arr, 1)
						for i,v in ipairs(arr) do arr[i] = { id = v.id, entity = v.raw_entity, coord = v.coord, num = (v.num ~= 0 and v.num or nil) } end
						regs[map].queue = arr
					end
					return true
				end
				...
				for _,comp in ipairs(entity_components) do
					...
					elseif base_id == "c_deployer" and comp.has_extra_data and include_internals then
						...
						if deployer_bp then bpcomp_3, bpcomp_4 = Tool.TableToString(deployer_bp), comp.extra_data.onetime end
					...
					local bpcomp_idx, relevant, comp_reg_idx, rdefs, inherent = (components and #components + 1 or 1), (not hidden or bpcomp_3), comp.register_index, def.registers
					for i=1,reg_num do
						relevant = AddReg(comp_reg_idx - 1 + i, string.format('%d|%d', bpcomp_idx, i), rdefs and rdefs[i] and rdefs[i].read_only) or relevant
					end

					-- Figure out if this specific component is inherited from the frame definition (and if not, make sure to store a hidden component in a blueprint with internals)
					if inherent_comps and (inherent_comps[id] or 0) > 0 then inherent_comps[id], inherent = inherent_comps[id] - 1, true end
					if include_internals and not inherent and not relevant then relevant = true end
					...
					-- Register relevant component (either is non-hidden, has meta data in bpcomp3, has registers/links or is non-inherent in a blueprint with internals)
					local bpcomp = { id, (inherent and 'inherent') or (hidden and "hidden") or comp.socket_index, bpcomp_3, bpcomp_4 }
...
function ApplyBlueprintToEntity(entity, bp)
	...
	for _,v in ipairs(data.logistics_flags) do
		local flag = v.flag
		if flag then
			if can_carry or (flag ~= 'carrier' and flag ~= 'transport_route' and flag ~= 'can_construction') then
				local val = bp_logistics and bp_logistics[flag]
				if val == nil then val = v.default end
				entity["logistics_" .. flag] = val
			end
		end
	end
...
function CreateFrameOrBlueprint(faction, def, include_internals, extra_data_collection, component_skip_list, dependency_need_import, replace_entity)
	...
	if entity and is_blueprint then
		...
		-- Blueprint including internals might not contain all inherent components (from the frame definition), so remove these components
		if include_internals and entity.component_count > 0 then
			local entity_components = entity.components
			if bp_components then
				for _,v in ipairs(bp_components) do
					if v[2] == 'inherent' then
						for i,c in ipairs(entity_components) do
							if c.id == v[1] then
								entity_components[i] = entity_components[#entity_components]
								entity_components[#entity_components] = nil
								break
							end
						end
					end
			...
			-- Remove non-hidden inherent components not in the blueprint with internals
			for i,c in ipairs(entity_components) do if not c.is_hidden then c:Destroy(true) end end
		end
			...
			if bp_components then
				for bpcomp_idx,v in ipairs(bp_components) do
					...
					elseif payload_base_id == "c_deployer" and include_internals then
						ed = { bp = (type(payload) == "string" and Tool.StringToTable(payload) or Tool.Copy(payload)), onetime = v[4] }
