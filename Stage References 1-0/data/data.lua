...
function package:on_update(ver)
	if ver == 8 then -- Calculate transfer boost for Item Transporters
		for _,comp in ipairs(Map.GetComponents("c_crane", true)) do
			comp.owner.transfer_boost = comp.owner.transfer_boost + 1
		end
	elseif ver == 7 then -- Removed packages, replace them with one-time-use deployers
...
function package:get_new_player_faction_id()
	...
	if Map.GetSettings().game_mode == "Separate/Versus" then
		...
		local old = Map.GetFaction("faction_1") -- compatibility with 1.0 bug
		if not old or not old.extra_data.locked then return "faction_1" end
		for i=1,99999 do
			local faction_id = i == 1 and "player" or ("player_" .. i)
