function PlayerAction.PauseGame(player_id, faction, arg)
	...
	if arg.step then Map.Delay("SimulationStep", 0) end -- delay 0 can be used in action functions
end

function Delay.SimulationStep()
	Map.SetGameSpeed(0)
end
...
function EntityAction.InvToComp(entity, arg)
	...
	if oldcomp then
		...
		if slot.locked and slot.id ~= oldcomp.id then print("cannot unequip component into locked slot") return end
...
function EntityAction.CompToInv(entity, arg)
	...
	if comp.is_hidden then print("cannot unequip hidden component") return end
	...
	if slot.locked and slot.id ~= comp.id then print("cannot insert component into locked slot") return end
...
function EntityAction.InvToInv(entity, arg)
	...
	if slot1.type ~= slot2type then
		...
		local deployer_bp_def = deployer_ed and deployer_ed.onetime and deployer_ed.bp and data.frames[deployer_ed.bp.frame]
		local pack_satellite = slot1.def.flags == "Space" and slot2.type == "storage" and slot1.entity
		...
		elseif deployer_ed then
			...
			slot2:SetItemAndStack('c_deployer', 1, { bp = MakeBlueprintFromEntity(pack_satellite, false, true), onetime = true })
			for _,v in ipairs(pack_satellite.components or {}) do v:Destroy(true) end -- don't drop components
			pack_satellite:Destroy(entity)
		end
	...
	for _,comp in ipairs(entity.components or {}) do
		...
		elseif (comp_base_id == "c_turret" or comp_base_id == "c_hacking_tool") and comp:GetRegisterId(1) ~= "v_powereddown" then
			...
			if not data.frames[reg1_id] then
				setreg, relevant = true, target_entity and entity.faction:IsEnemy(target_entity)
			end
		end
...
local function fill_upgrade_inventory(entity, upgrade_inventory, remove_unreserved)
	local slots = entity.slots
	if not slots then return end
	for _,slot in ipairs(slots) do
		local amount = (remove_unreserved and slot.unreserved_stack or slot.stack)
		if amount > 0 then
			...
			elseif slotent then
				slotent:Place(entity)
				upgrade_inventory[#upgrade_inventory+1] = slotent
			else
				local has_extra_data = amount == 1 and slot.has_extra_data
				upgrade_inventory[#upgrade_inventory+1] = { slot.id, amount, has_extra_data and slot.extra_data or nil }
				if remove_unreserved then
					if has_extra_data then
						slot:Clear()
					else
						slot:RemoveStack(amount)
					end
				end
			end
		end
...
function FactionAction.PlaceConstruction(faction, arg)
		...
		if upgrade_buildings or dropped_items then
			...
			if dropped_items then for _,e in ipairs(dropped_items) do
				if e.id == "f_dropped_item" then -- don't take in scattered resources
					fill_upgrade_inventory(e, upgrade_inventory, true) -- destroys dropped item if emptied
				end
			end end
		end
	...
	if frame_id then
		...
		for i,loc in ipairs(locations) do
			...
			for _,m in ipairs(multi) do
				local m_placed = m.bp and m.placed
				local m_regs = m_placed and m_placed.extra_data.custom_blueprint.regs
				if m_regs then
...
local function Relocate_Entity(faction, e, bp, x, y, rotation, multis)
	...
	if upgrades and (#upgrades == 1 and upgrades[1] == e) and org_location.x == x and org_location.y == y then
		if e.def.try_rotate or e.rotation == rotation or e:SetVisual(e.visual_id, rotation) then
			return
		end
	end
...
function FactionAction.Relocate(faction, arg)
	...
	if not entities then
		...
		if newe then Relocate_Finish(newe, x, y, rotation, deployer_ed) end
	else
