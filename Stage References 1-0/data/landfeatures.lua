...
local crystal_richness_min = math.floor(80*rich_mul)
local crystal_richness_max = math.floor(88*rich_mul)
...
local metal_richness_min = math.floor(400*rich_mul) -- 60
local metal_richness_max = math.floor(460*rich_mul) -- 80
...
local silica_richness_min = math.floor(180*rich_mul) -- 30
local silica_richness_max = math.floor(200*rich_mul) -- 40
local silica_falloff = (plateau_center-plateau_rocky)/2.0
local silica_patch_size = 0.05
local silica_patch_edge = 0.4

local laterite_richness_min = math.floor(360*rich_mul)
local laterite_richness_max = math.floor(500*rich_mul)

local blight_crystal_patch_falloff = 0.03
local blight_crystal_richness_min = math.floor(40*rich_mul) -- 2
local blight_crystal_richness_max = math.floor(48*rich_mul)-- 4

local obsidian_richness_min = math.floor(80*rich_mul)
local obsidian_richness_max = math.floor(100*rich_mul)
...
local resources = {
	...
	-- silica plateau
	{
		...
		functions = {
			{ func = "Threshold", param = "Blightness", range = { -1, blight_threshold } },
			{ func = "Threshold", param = "Elevation", range = { plateau_level+silica_falloff, plateau_level+(silica_falloff*2.0)+1 }, falloff=silica_falloff },
...

	{
		...
		functions = {
			{ func = "Threshold", param = "Blightness", range = { -1, blight_threshold } },
			{ func = "Threshold", param = "Elevation", range = { plateau_level, plateau_level+(silica_falloff*2.0)+1 }, falloff=silica_falloff },
			{ func = "Threshold", param = "Variation", range = { silica_patch_edge-silica_patch_size, silica_patch_edge }, falloff = 0.15, },
