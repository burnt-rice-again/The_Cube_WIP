...
data.components.samplecomponent = {
	...
	on_take_damage = function(self, comp, amount, damager) ... end,
...
local function SumModuleBoosts(owner, id, remove_comp)
	local sum = 0
	for i=1,999 do
...
c_fabricator:RegisterComponent("c_assembler", {
	...
	power = -10,
	production_recipe = CreateProductionRecipe({ metalplate = 5, crystal = 10 }, { c_fabricator = 40, c_assembler = 40, c_human_factory = 80 }),
	link_to_visual = true,
...
function c_repairer:on_trigger(comp, other_entity)
	if not comp.is_working and comp.faction:IsAlly(other_entity) then
		comp:Activate()
	end
end
...
c_repairkit:RegisterComponent("c_giga_regeneration", {
	attachment_size = "Hidden",
	production_recipe = false,
	power = 0,
	repair = 50,   -- repair health per use
})
...
function c_deployer:action_click(comp)
	...
	if not bp then
		...
		View.StartCursorConstruction("f_construction", bp.visual or bp.frame,
			function(location, rotation, is_valid) -- on confirm
				if not is_valid and comp.exists then Notification.Error("Cannot deploy here") return end -- continue cursor
				View.StopCursor()
				Quickview_HideGrid()
	...
	if cause & CC_FINISH_WORK == CC_FINISH_WORK then
		...
		Map.Defer(function()
			...
			if not is_upgrade then
				...
				local fsize = frame_def.size
				if fsize ~= "Other" and fsize ~= "Wall" and fsize ~= "Foundation" then
...
local c_deconstructor = Comp:RegisterComponent("c_deconstructor", {
	...
	-- internal variable
	charge_time = 10, -- deconstruct duration
})
...
function c_deconstructor:on_update(comp, cause)
	...
	return comp:SetStateStartWork(self.charge_time, true, (cause & CC_CHANGED_REGISTER_ENTITY == 0))
end
...
local c_turret = Comp:RegisterComponent("c_turret", {
	...
	charge_time = 6, -- charge duration -- 2
...
function c_turret:on_trigger(comp, other_entity)
	if comp.faction:IsEnemy(other_entity) then
		comp:Activate()
	end
end
...
local function TurretApplyDamage(compdef, comp, enemy, damage, damage_type, damager, extra_effect)
	...
	if enemy.exists and enemy.health > 0 then
		if extra_effect then extra_effect(compdef, comp, enemy) end
	elseif not comp:RegisterIsLink(1) and comp:GetRegister(1).raw_entity == enemy then
		comp:SetRegister(1, nil)
	end
end
...
function c_turret:damage_func(comp, e, trgloc)
	...
	if not e or not e.exists or e:GetRangeSquaredTo(trgloc) > 4 then
		...
		if not e or not damager_faction:IsEnemy(e) then
			e = nil
		end
	end
	if e then
	...
	elseif self.pulse then
		local affects_flying = self.affects_flying
		for _,enemy in ipairs(damager.is_on_map and Map.GetEntitiesInRange(damager, self.pulse, FF_OPERATING|FF_WALL|FF_GATE|FF_ENEMYFACTION) or {}) do
	...
	elseif self.beam_range then -- beam style (railgun)
		for _,enemy in ipairs(damager.is_on_map and Map.GetEntitiesOnLine(damager, trgloc, self.beam_range, FF_OPERATING|FF_WALL|FF_GATE|FF_ENEMYFACTION) or {}) do -- TODO: needs to specifically add in the target entity
...
local c_portable_turret = c_turret:RegisterComponent("c_portable_turret", {
	...
	damage_type = "energy_damage",
	charge_time = 5, -- charge duration -- 3
	shoot_while_moving = true,
...
c_turret:RegisterComponent("c_adv_portable_turret", {
	...
	damage_type = "energy_damage",
	charge_time = 6, -- charge duration -- 3
	shoot_while_moving = true,
	shoot_fx = "fx_turret_laser",
	shoot_speed = 0,
})
...
c_portable_turret:RegisterComponent("c_portable_turret_green", {
	...
	damage_type = "energy_damage",
	charge_time = 8, -- charge duration -- 4--6
	-- shoot_while_moving = false,
	shoot_fx = "fx_turret_laser",
	shoot_speed = 0,
...
c_turret:RegisterComponent("c_laser_turret", {
	...
	damage_type = "energy_damage",
	charge_time = 6, -- charge duration -- 6
	shoot_while_moving = true,
	shoot_fx = "fx_turret_laser",
})
...
c_turret:RegisterComponent("c_plasma_cannon", {
	...
	charge_time = 25, -- charge duration -- 2
	shoot_speed = 9,
	damage_type = "plasma_damage",
	blast = 2,
	shoot_while_moving = false,
...
c_turret:RegisterComponent("c_photon_beam", {
	...
	damage_type = "energy_damage",
	charge_time = 10, -- charge duration -- 10
...
c_turret:RegisterComponent("c_photon_cannon", {
	...
	damage_type = "energy_damage",
	charge_time = 20, -- charge duration -- 10
	------ CANNON -------
	shoot_speed = 10,
	blast = 2,
	shoot_while_moving = false,
...
c_turret:RegisterComponent("c_missile_turret", {
	...
	charge_time = 30, -- charge duration -- 30
...
c_turret:RegisterComponent("c_pulse_disrupter", {
	...
	damage_type = "energy_damage",
	charge_time = 8, -- charge duration -12
	shoot_while_moving = true,
	shoot_fx = "fx_pulse",
	shoot_socket = '_entity',
	pulse = 1,
...
c_turret:RegisterComponent("c_pulselasers", {
	...
	damage_air_bonus = 2,
	damage_type = "energy_damage",
	charge_time = 1, -- charge duration --2
	shoot_while_moving = true,
...
c_turret:RegisterComponent("c_twin_autocannons", {
	...
	desc = "Short-range twin-autocannons. Effective against air targets.",
	power = -10, -- --10
	...
	damage_type = "physical_damage",
	charge_time = 1, -- charge duration --2--4
	shoot_while_moving = true,
...
c_turret:RegisterComponent("c_railgun", {
	...
	damage_type = "physical_damage",
	charge_time = 12, -- charge duration --15
	shoot_fx = "fx_railgun",
	beam_range = 10,
	damage_air_bonus = 0.5,
	shoot_while_moving = false,
...
c_turret:RegisterComponent("c_plasma_turret", {
	...
	damage_type = "plasma_damage",
	charge_time = 3, -- charge duration -- 50
	shoot_speed = 1,
	shoot_while_moving = true,
...
c_turret:RegisterComponent("c_hybrid_beam_cannon", {
	...
	damage_type = "plasma_damage",
	charge_time = 15, -- charge duration --15
...
c_turret:RegisterComponent("c_light_cannon", {
	...
	desc = "Long-range recoilless rifle",
	power = -10,
...
c_turret:RegisterComponent("c_human_missilelauncher", {
	...
	charge_time = 18, ---- charge duration
	damage_type = "physical_damage",
	shoot_fx = "fx_turret_missile",
	shoot_speed = 10, -- delay between shoot and hit
	blast = 2, -- splash range
...
c_turret:RegisterComponent("c_human_supportlauncher", {
	...
	charge_time = 30, ---- charge duration
	damage_type = "physical_damage",
	shoot_fx = "fx_turret_missile",
...
c_capacitor:RegisterComponent("c_higrade_capacitor", {
...
c_capacitor:RegisterComponent("c_medium_capacitor", {
...
----- small battery -----
local c_small_battery = Comp:RegisterComponent("c_small_battery", {
...
local c_signal_reader = Comp:RegisterComponent("c_signal_reader", {
	...
	registers = {
		...
		{ tip = "<header>RECEIVED SIGNAL</>\n\n<bl>The Result returned from the Target's </><hl>Signal</><bl> register</>", ui_icon = "icon_signal", read_only = true }
	},
...
local c_internal_storage = Comp:RegisterComponent("c_internal_storage", {
	...
	get_ui = function(def, comp)
		local newui = UI.New([[
			...
			on_click_behavior_modify = function(b, btnmodify)
				...
				UI.AddLayout("Program", { comp = comp, library_item = asm.code, is_remote = true, library = Game.GetLocalPlayerFaction().extra_data.library }, 1)
			end,
...
local function c_behavior_on_end(comp, state, asm)
	...
	elseif #asm > 0 then
		...
		state.counter, state.limit, state.unlock_continue = 1, nil
		return asm, true
	end
end
...
function c_behavior:on_update(comp, cause)
	...
	local debug, revid, step, step_limit, breakpoints, restart = state.debug, state.revid, 1
	...
	while true do
		...
		while not inst do
			asm, restart = c_behavior_on_end(comp, state, asm)
			if not asm then goto restart_changed_code end
			if restart and step > 1 then comp:SetStateSleep(1) goto pause_on_restart end
		...
		if step >= step_limit then
			if debug then
				...
				if not breakpoints then break end -- pause execution when stepping
				if (state.counter ~= state.lastcounter or state.revid ~= revid) then
					while asm and not asm[state.counter] do
						asm, restart = c_behavior_on_end(comp, state, asm)
						if restart and step_limit > 1 then comp:SetStateSleep(1) goto pause_on_restart end
					end
				...
				if step - 1000000000 < step_limit then goto debug_continue_unlocked end -- no breakpoint hit, continue unlocked behavior normally
			end

			if step_limit == 1 or state.unlock_continue then
				comp:SetStateSleep(1)
				break
			else
				InstError(comp, state, "Unlocked behavior exceeded instruction limit", true)
				return
			end
	...
	::pause_on_restart::
	if not debug then
		state.laststep, state.lasttick = step, Map.GetTick()
		return
	end
...
function c_crane:on_add(comp)
	...
	owner.transfer_boost = owner.transfer_boost + 1
end
...
function c_portable_radar:on_update(comp, cause)
	...
	if filters[1] == "v_mothership" and (comp.id == "c_radar" or comp.id == "c_radar_array" or comp.id == "c_radar_suite") then
...
function c_alien_powercore:on_update(comp, cause)
	...
	local has_power = owner.powered_down or owner.is_docked or (owner.has_power and comp.stored_power > 1500) or owner.faction.is_world_faction
...
local function get_trilobyte_consume_rewards(id, num, tbl)
	...
	for k,v in pairs(tbl or {}) do
		...
		if item_def and (item_def.slot_type == "anomaly" or item_def.slot_type == "gas") then
			remove[k] = true
		end
	end
...
function c_trilobyte_attack:on_take_damage(comp, amount, damager)
	...
	if not self.ignore_damagers and damager and damager.exists then
		local reg1 = comp:GetRegister(1)
		if not reg1.is_link then
			reg1 = reg1.entity
			if not reg1 then
				comp:SetRegisterEntity(1, damager)
			elseif reg1 ~= damager and reg1:GetRangeSquaredTo(comp) > damager:GetRangeSquaredTo(comp) then
				comp:SetRegisterEntity(1, damager)
			end
		end
	end
...
c_trilobyte_attack:RegisterComponent("c_tripodonte1", {
	...
	on_take_damage = function(self, comp, damage, damager, damage_type)
		...
		Map.Defer(function() if comp.exists and damager.exists then self:on_trigger(comp, damager) end end)
	end,
})
...
c_trilobyte_attack:RegisterComponent("c_wasp_attack1", {
	...
	charge_time = 10, -- charge duration
	shoot_speed = 4,
	leash_distance = 40,
...
c_internal_storage:RegisterComponent("c_small_storage", {
...
Comp:RegisterComponent("c_bug_homeless", {
	...
	on_take_damage = function(self, comp, amount, damager)
		...
		if turret then
			...
			if not reg.entity and damager and damager.exists then
...
local c_bug_spawn = Comp:RegisterComponent("c_bug_spawn", {
	...
	on_trigger_action = function (self, comp, other_entity, force)
		...
		if not other_entity.faction.is_player_controlled or not owner_faction:IsEnemy(other_entity) or other_entity.stealth then
			return
		end
...
c_bug_spawn:RegisterComponent("c_bug_spawner_large", {
	...
	on_update = function(self, comp, cause)
		...
		if bugs_faction.num_entities > (3000 * Map.GetPlayerFactionCount()) then return comp:SetStateSleep(1000) end
		...
		local time_between = Map.GetTick() - last_swarm
		if time_between < 700 then
			return comp:SetStateSleep(time_between+1)
		end
		...
		if extra_data.extra_spawned > 10 then
			...
			if rnd < 0.2 then
				...
				local hivecount = 0
				local found = Map.FindClosestEntity(comp.owner, 10, function(enemy)
					if enemy.id == "f_bug_hive" or enemy.id == "f_bug_hive_large" then
			...
			elseif rnd > 0.3 then
				...
				for _, faction in ipairs(Map.GetFactions()) do
					if faction.is_player_controlled and faction.num_entities > 0 and bugs_faction:IsEnemy(faction) then
						local newdist = 9999998
						local test_entity
						local tries = 1
						while not test_entity do
							test_entity = faction.entities[math.random(1, faction.num_entities)]
							if test_entity.is_docked then
								test_entity = test_entity.docked_garage
							end
							if test_entity and not test_entity.is_placed then
								test_entity = nil
							end
							tries = tries + 1
							if tries > 20 then
				...
				if closest_faction then
					...
					if ((peaceful == 2 and closest_distance > 20) or (closest_distance > 100)) and (bugs_faction.num_entities < (2000* Map.GetPlayerFactionCount())) then
						local rnd = math.random()
						if rnd > 0.6 then
							-- spawn a triloscout
							Map.Defer(function()
								...
								if rnd > 0.5 or bugs_faction.num_entities > 200 then -- move it towards the enemy
					...
					elseif peaceful == 3 or (closest_distance <= 45) then -- only trigger attacks forcefully in aggressive mode or if really close in hostile
						-- trigger assault
						...
						if ent then
							--print("triggering", comp.owner.location, Map.GetTick())
		...
		elseif state == "deploy" then
			...
			Map.Defer(function()
				if comp.exists then
					-- check local area
					local holes = 0
					local nearbyhive = Map.FindClosestEntity(comp.owner, 20, function(e)
						if e.id == "f_bug_hive_large" then
							return true
						end
						if e.id == "f_bug_hole" then
							holes = holes + 1
						end
					end, FF_OPERATING|FF_OWNFACTION)
					local rnd = math.random()
					if not nearbyhive or (holes < 10 and rnd > 0.5) then
						local newhome = Map.CreateEntity(GetBugsFaction(), (nearbyhive or (rnd > 0.8)) and "f_bug_hole" or "f_bug_hive_large")
						newhome:Place(owner.location)
					end
					comp.extra_data.extra_spawned = 0
					owner:Destroy()
...
local c_virus = Comp:RegisterComponent("c_virus", {
	...
	on_trigger = def_comp_activate, -- infect on trigger
...
function c_small_scanner:on_update(comp, cause)
	...
	comp:SetRegister(2, nil)
	return comp:SetStateStartWork(self.charge_time * (scannable.faction.is_world_faction and 1 or 4), true, (cause & CC_CHANGED_REGISTER_ENTITY == 0))
end
...
Comp:RegisterComponent("c_alien_sc", {
	...
	charge_time = 5 * TICKS_PER_SECOND,
	get_reg_error = function(self, comp)
		if not comp.owner:FindComponent("c_resimulator", true) then return "Must be equipped on a Re-simulator" end
	...
	on_update = function(self, comp, cause)
		local owner = comp.owner
		if not owner:FindComponent("c_resimulator", true) then comp:FlagRegisterError(1) return end
		...
		return comp:SetStateStartWork(self.charge_time, true)
	end,
})
...
Comp:RegisterComponent("c_synthesis_pool", {
	...
	synthesis_s_comp_cost = 20,
	synthesis_i_comp_cost = 10,
	charge_time = 25 * TICKS_PER_SECOND,
...
local function get_work_time(comp, base_work_time, is_blight_boost)
	local owner, socket_index = comp.owner, comp.socket_index
	local socket_boost = socket_index > 0 and GetAttachmentSize(owner.visual_def.sockets[socket_index][2]) > GetAttachmentSize(comp.def.attachment_size)
	local eff_boost = owner.component_boost + owner.faction.component_boost - 100 + (socket_boost and 50 or 0)
	local work_time = base_work_time
	if is_blight_boost and Map.GetBlightnessDelta(comp, -1) >= 0 then work_time = work_time // 2 end
	local tick_boost = 0
	if eff_boost ~= 100 or work_time ~= base_work_time then tick_boost = (work_time * 100 + eff_boost - 1) // eff_boost end
...
c_fabricator:RegisterComponent("c_alien_factory_robots", {
	...
	power = -500,
	production_recipe = CreateProductionRecipe({ obsidian_brick = 20, reinforced_plate = 10, blight_plasma = 10, }, { c_advanced_assembler = 150, c_reforming_pool = 100, c_reforming_pool_comp = 100 }),
})
...
function c_virus_cure:on_trigger(comp, other_entity)
	...
	if virus_comp ~= nil then
		...
		if other_entity.id == "f_exploreable_bot_glitch" then
			...
			other_entity.has_blight_shield = true
		end
...
----- large_storage -----
c_internal_storage:RegisterComponent("c_large_storage", {
...
function c_unit_teleport:on_update(comp, cause)
	...
	for i,v in ipairs(comp.slots) do
		...
		local valid_faction = target_entity and target_entity.faction:IsAlly(comp.faction)
	...
	if cause & (CC_FINISH_WORK) == CC_FINISH_WORK then
		for i,v in ipairs(comp.slots) do
			...
			if docked_entity then
				...
				docked_entity.health = math.min(docked_entity.health + self.repair, docked_entity.max_health)
			end
		end
	end
	...
	for i,v in ipairs(comp.slots) do
		...
		if docked_entity then
			if docked_entity.is_damaged then
				return comp:SetStateStartWork(self.charge_time, false)
			end
		end
	end
	return comp:SetStateSleep()
...
function c_hacking_tool:on_update(comp, cause)
	...
	if cause & (CC_FINISH_WORK | CC_CHANGED_REGISTER_ENTITY) == CC_FINISH_WORK then
		...
		if target_entity.id == "f_alien_console" then
			local race = comp.faction.extra_data.race or "robot"
			if race ~= "alien" then
				comp.faction:Unlock("t_robots_alien_discovery")
			end
		end
	...
	return comp:SetStateStartWork((target_entity.id == "f_charcharosaurus1" and 3 or 1) * self.charge_time, true, (cause & CC_CHANGED_REGISTER_ENTITY == 0))
end
...
function c_blight_power:on_update(comp, cause)
	...
	-- reserve or order 2 so 1 can be consumed immediately and 1 is kept reserved
	if in_blight and comp:PrepareConsumeProcess({ blight_extraction = 1 }, 2) then
		comp:FulfillProcess()
		if not comp.has_active_effects and comp.owner.is_placed then
			comp:PlayEffect("fx_blight_power")
		end
		comp.extra_power = 200
		comp:SetRegister(1, { id = "v_power_production", num = ((200 + (self.power or 0)) * TICKS_PER_SECOND) })
		return comp:SetStateStartWork(75)
	elseif not in_blight then
		comp:CancelProcess()
	end
	comp:StopEffects()
	comp.extra_power = 0
...
----- battery -----
c_small_battery:RegisterComponent("c_battery", {
...
----- large battery -----
c_small_battery:RegisterComponent("c_large_battery", {
...
c_internal_storage:RegisterComponent("c_medium_storage", {
...
function c_blight_shield:on_remove(comp)
	local owner = comp.owner
	if owner:CountComponents("c_blight_shield", true) == 1 then owner.has_blight_shield = (owner.def.start_blight_shield or false) end
end
...
c_portable_radar:RegisterComponent("c_alien_sensor", {
	...
	-- lua variable
	range = 35, -- scan distance
	radar_show_range = 8, -- reveal area
	charge_time = 5,
	radar_show_area = true,
})
...
c_small_radar:RegisterComponent("c_alien_sensor_wide", {
	...
	-- lua variable
	range = 60, -- scan distance
	radar_show_range = 12, -- reveal area
	charge_time = 3,
	radar_show_area = true,
})
...
local c_terraformer = Comp:RegisterComponent("c_terraformer", {
	...
	terraforming_target = Map.GetSettings().blight_threshold-0.3,
	activation = "Always",
...
local c_virus_bitlock = c_turret:RegisterComponent("c_virus_bitlock", {
	...
	charge_time = 25, -- charge duration
...
local c_warp_bridge = Comp:RegisterComponent("c_warp_bridge", {
	...
	charge_time = 30 * TICKS_PER_SECOND, -- cast charge duration
...
function c_warp_bridge:on_update(comp, cause)
	...
	if cause & CC_FINISH_WORK == 0 then
		...
		return comp:SetStateStartWork(self.charge_time, true, (cause & CC_CHANGED_REGISTER_COORD == 0)) -- charge
	end
	...
	if owner.health <= 0 then
		...
		Map.Defer(function()
			if owner.exists then
				if owner.is_docked then owner:Undock() end
				owner:Unplace()
			end
			if old_entity.exists then
				old_entity:Place(place_at)
			end
		end)
	...
	for k,v in ipairs(comp.owner.slots or {}) do
		if v.id then
			...
			if v.unreserved_stack > 0 and data.components[v.id] then
				-- also check if its a deployer
				if v.id ~= "c_deployer" then
...
function c_virus_possessor:on_update(comp, cause)
	...
	if cause & (CC_FINISH_WORK | CC_CHANGED_REGISTER_ENTITY) == CC_FINISH_WORK then
		...
		Map.Defer(function()
			...
			if not target.is_on_map then return end
...
Comp:RegisterComponent("c_bug_wave_spawner", {
	...
	desc = "Clones waves of bugs into existence by replicating existing copies of the virus. A temporary wave of bugs will be spawned when provided with the required resources.",
	visual = "v_generic_i",
	activation = "OnFirstRegisterChange",
	get_ui = true,
	registers = {
...
local c_blight_magnifier = Comp:RegisterComponent("c_blight_magnifier", {
	...
	desc = "Slowly regenerates nearby resources up to 200, only works if placed inside the blight. Magnification can be boosted with Anomaly Particles.",
	range = 2,
	power = -100,
	...
	return UI.New([[
		...
		tooltip = function(w)
			w.tt = UI.New("<Box bg=popup_box_bg blur=true padding=12><HorizontalList child_align=center child_padding=10><Reg entity={entity} bg=card_box_bg def_id={def_id} num={num}/><Text size=12 text={txt}/></HorizontalList></Box>", {
				...
				destruct = function()
					if w:IsValid() then w.tt = nil end
				end,
			})[1]
			w:update()
			return w.tt.parent
...
local c_turret_phaseflower = c_turret:RegisterComponent("c_turret_phaseflower", {
	...
	charge_time = 16, -- charge duration
...
Comp:RegisterComponent("c_alien_core", {
	texture = "Main/textures/icons/components/int.png",
...
-------------------------------------------
----- alien_miner -----
c_miner:RegisterComponent("c_alien_miner", {
...
c_blight_extractor:RegisterComponent("c_alien_feeder", {
...
Comp:RegisterComponent("c_anomaly_event", {
	activation = "Always",
	effect = "fx_glitch",
	on_take_damage = function(self, comp)
...
function c_time_egg_transference:on_update(comp, cause)
	...
	for _,slot in ipairs(owner.slots) do
		if slot.unreserved_stack > 0 and target:HaveFreeSpace(slot.id) and not slot.entity then
...
c_small_radar:RegisterComponent("c_sensor_spike_comp", {
	...
	-- lua variable
	range = 50, -- scan distance
	radar_show_range = 10, -- reveal area
	charge_time = 5,
	radar_show_area = true,
