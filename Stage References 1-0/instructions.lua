...
{
	func = function(comp, state, cause, in_producer, out_item, exec_done)
		local producer_id = GetId(comp, state, in_producer)
		local it = { 1 }
		local def, recipe
		for i,k in ipairs(comp.faction.unlocked_items) do
			def = data.items[k]
	...
	next = function(comp, state, it, in_producer, out_item, exec_done)
		local i = it[1]
		i = i + 1
		if i > #it then return true end
	...
	last = function(comp, state, it, in_producer, out_item, exec_done)
...
local stats_item = {
	...
	{ function(def) return def.range or def.attack_radius or def.transfer_radius end, "Range" },
...
data.instructions.get_closest_entity =
{
	func = function(comp, state, cause, f1, f2, f3, output)
		...
		local res, num = Map.FindClosestEntity(owner, math.min(override_range or range, range), function(e) return FilterEntity(owner, e, filters) end, entity_filter)
