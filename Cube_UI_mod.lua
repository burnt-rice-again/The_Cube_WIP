		...
		<VerticalList child_padding=2 on_mouse_wheel={on_value_mouse_wheel} margin_top=10>
			<Box id=tabbox bg=popup_additional_bg padding=6>
				<HorizontalList id=tabs child_padding=5 child_fill=true>
					...
					<Button text="Unit Types"     tab=frame  on_click={tab_on_click}/>
			...
			<Box bg=popup_additional_bg padding=6 id=production hidden=true>
				<HorizontalList halign=right child_align=top>
					...
					<Text width=56 y=10 text="→dd" size=20  textalign=center/>
...
local function FillDB(rs)
	...
	local on_double_click = rs.on_ui_accept and function() rs:on_ui_accept() end
	...
	local function add_definition(id, def, category, bp_frame_def, bp_multi, catidx)
		...
		if not bp_frame_def then
			tab_db[#tab_db+1] = {
				...
				sort_key = string.format("%03d%03d%s", catidx, RaceTagOrder[def.race or def.tag] or 999, id),
...
function RegisterSelection:construct()
	...
	if comp then
		...
		any_value = (comp.base_id == "c_behavior" or Input.IsControlDown())
		register_def = not any_value and comp_def.registers and comp_def.registers[self.index]
		self.entity = entity
	else
		comp_def, register_def = self.comp_def, self.register_def
		any_value = (self.index and self.index <= FRAMEREG_COUNT) or (comp_def and comp_def.base_id == "c_behavior") or self.any_value
	end

	local title1, title2 = comp_def and comp_def.name, register_def and register_def.tip
	if title1 or title2 then
		self.title.text = ((title1 and title2 and L("%s - %s", title1, title2)) or title1 or title2 or "")
	else
...
function RegisterSelection:RefreshTab(new_tab, filter)
	...
	if selectbtn then
		...
		local new_tech, listed_defs, last_cat_name, regs_list = extra.new_tech, {}
		self.list:Clear()
...
function RegisterSelection:on_library_loaded(btn, bp)
	...
	if w then self:SetDefId(w.bp.frame, w) end
end
...
function RegisterSelection:edit_on_click(btn)
	local pop = UI.MenuPopup("<Box bg=popup_box_bg padding=4 blur=true/>", { destruct = function() if btn:IsValid() then btn.active = false end end }, btn)
	if not pop then return end
	btn.active = true
	local source_bp = self.selected_bp or { frame = self.select.def.id }
	pop:Add("BlueprintEditor", {
		source_bp = source_bp, bp = self.edited_bp, library = self.library or {}, is_remote = true, want_similar = true,
		on_ok = function(pp) UI.CloseMenuPopup(pp) end,
		on_change = function(pp, bp) if self:IsValid() then self:UpdateVisuals(false, true, bp, self.selected_bp, self.library_id) end end,
	})
end

function RegisterSelection:SetDefId(id, bp_widget, in_num, is_right_click)
	local reg = self.register
	local new_tech = Game.GetLocalPlayerExtra().new_tech
	if id and new_tech then
		new_tech[id] = nil
	end
	...
	if self.is_production and id then
		...
		local init_num = item_def and item_def.stack_size and item_def.stack_size > 1 and recipe_amount == 1 and not item_def.convert_to and REG_INFINITE
	...
	reg.id = id
	self:UpdateVisuals(true, true, nil, (bp_widget and bp_widget.bp), (bp_widget and bp_widget.library_id))
...
function RegisterSelection:on_ui_accept()
	...
	if self.index then
		local entity, arg = self.entity, { idx = self.index, reg = reg, comp = self.component or nil }
		local bp = self.edited_bp or (library_id and library_id ~= -1 and self.library[library_id])
		local clipboard_bp = library_id and not bp and library_id <= -1 and UnitCopyPaste.GetBlueprint()
		if clipboard_bp then
			UILibraryImportBlueprint(clipboard_bp, function(res) -- self is now invalid inside this callback
				arg.custom_blueprint = res Action.SendForEntity("SetRegister", entity, arg)
			end)
		elseif bp then
			ProcessLibraryBlueprint(bp, function(bp) arg.custom_blueprint = bp Action.SendForEntity("SetRegister", entity, arg) end)
		elseif not library_id then
			Action.SendForEntity("SetRegister", entity, arg)
		end
	end
